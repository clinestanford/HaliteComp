...
def shipActions(team_ships,game_map):
    ...
    for ship in team_ships:
        ...
        for planet in closest_planets:
            if(not planet.is_owned() or (not planet.is_full() and ship.owner == planet.owner)):
                closest_good_planets.append(planet)
        closest_friendly_ships = [entities_by_distance[distance][0] for distance in entities_by_distance if isinstance(entities_by_distance[distance][0], hlt.entity.Ship) and entities_by_distance[distance][0] in team_ships]
