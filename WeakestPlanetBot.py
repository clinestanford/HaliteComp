...
logging.info("Starting WeakestPlanetBot!")
...
while True:
	...
	for ship in teamShips:
		...
		#if you have 8 or fewer ships, go mine other empty planets to make more		
		if len(teamShips) <= 8:
			if len(closestEmptyPlanets) > 0:
				planets = closestEmptyPlanets
			else: #if there are no empty planets, go attack the weakest planet anyway
				planets = sorted(closestEnemyPlanets, key=lambda x: len(x._docked_ship_ids), reverse=False)
				if len(planets) > 0:
		...
		else: #sort the closestEnemyPlanets by the number of ships docked on the planets in ascending order
			planets = sorted(closestEnemyPlanets, key=lambda x: len(x._docked_ship_ids), reverse=False)
			if len(planets) > 0:
					firstIds = planets[0]._docked_ship_ids
					lastIds = planets[len(planets)-1]._docked_ship_ids
		...
		if len(planets) > 0:
			#target the planet with the fewest ships docked
		...
		#this statement will be hit if you have taken all planets 
